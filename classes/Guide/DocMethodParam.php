 ...
 */
class DocMethodParam extends Injectable
{
    ...
    public function __toString()
    {
        ...
        if ($this->type) {
            $display .= '<small>'.$this->userguide->classLink($this->type).'</small> ';
        }
        ...
        if ($this->description) {
            $display .= '<span class="param" title="'.
                preg_replace('/\s+/', ' ', $this->description).'">$'.$this->name.'</span> ';
        } else {
            $display .= '$'.$this->name.' ';
        }
